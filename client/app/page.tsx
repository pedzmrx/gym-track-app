...
export default async function DashboardPage() {
  ...
  const { logsRecentas, proximoTreino } = await getDashboardStats(userId);
  ...
  return (
    <main className="min-h-screen bg-black text-white p-6 pb-44 max-w-md mx-auto">
      ...
      <section className="mb-10 px-1">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="bg-orange-500/10 border border-orange-500/20 p-3 rounded-2xl flex items-center gap-2">
            ...
            <span className="text-lg font-black italic text-orange-500">{logsRecentas.length}</span>
          </div>
        </div>
      </section>
      ...
      <section className="px-1 mb-20">
        ...
        {proximoTreino ? (
          <Link href={`/treinos/${proximoTreino.id}/executar`} className="relative block bg-zinc-900 border border-zinc-800 rounded-[2.5rem] p-8 overflow-hidden group active:scale-[0.98] transition-all shadow-sm">
              ...
              <div className="flex justify-between items-end">
                <div className="text-left">
                  ...
                  <p className="text-zinc-500 text-[10px] font-bold mt-2 uppercase tracking-widest">
                    {proximoTreino._count?.exercises || 0} exercícios • Foco Total
                  </p>
                </div>
            ...
            {/* Foto de fundo aleatória e dinâmica */}
